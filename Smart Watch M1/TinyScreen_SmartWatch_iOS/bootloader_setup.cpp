...
#define bootloader_debug_print false
...
{
  if(bootloader_debug_print)Serial.println(F("Jumping to bootloader"));
  delay(100);
    ...
    uint16_t adv_interval, uint8_t *pipes, uint8_t n_pipes)
{
  ...
  */
  ...
  {
    if(bootloader_debug_print)Serial.println(F("CRC does not match EEPROM. Writing new data."));
