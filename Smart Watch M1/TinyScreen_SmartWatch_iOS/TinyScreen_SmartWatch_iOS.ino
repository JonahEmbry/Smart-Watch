...
// Flashlight
uint8_t FlashlightBG = WHITE;
uint8_t FlashlightFont = BLACK;
...
//Maybe comment below to get watch to stop resetting time after restart?
...
// Maybe change the following to false?? Set to false -J 6/8 Did not work
uint8_t rewriteTime = false;
...
//Should edit to enable longer display time for notification??
void loop() {
  SPI.setBitOrder(LSBFIRST);
  ...
  //Puts display to sleep at 5 seconds -J 6/9
 ...
 // Turns on screen every 30 minutes to ensure Tinyduino doesn't fall asleep -J 6/8 Works
...
void ancs_notifications(ancs_notification_t* notif) {
  if ((notif->category == ANCS_CATEGORY_INCOMING_CALL) |
      ...
      //Added the following three notif -J 6/8
      (notif->category == ANCS_CATEGORY_VOICEMAIL)|
      (notif->category == ANCS_CATEGORY_OTHER)|
      (notif->category == ANCS_CATEGORY_SCHEDULE)) {
...
#else
// Add additional button significances here
...
const uint8_t flashButton = TSButtonUpperRight; //Flashlight Button
#endif
...
void buttonPress(uint8_t buttons) {
  if (currentDisplayState == displayStateHome) {
    if (buttons == viewButton) {
      ...
      menuHandler(0);
    } else if (buttons == flashButton){   //Flashlight Addition
      menuHandler = displayFlashlight;
      menuHandler(0);
    }    
...
// Flashlight
void displayFlashlight(uint8_t button) {
  if (!button) {
    if (menu_debug_print)Serial.println("viewNotificationsInit");
    currentDisplayState = displayStateMenu;
    display.clearWindow(0, 12, 96, 64);
    display.drawRect(0, 10, 100, 60, TSRectangleFilled, TS_8b_White);
    display.setFont(font10pt);
    display.fontColor(FlashlightFont, FlashlightBG);
    ...
    if (button == clearButton) {
      ...
      display.drawRect(0, 10, 100, 60, TSRectangleFilled, TS_8b_Black);
      currentDisplayState = displayStateHome;
      initHomeScreen();
    }
  }
